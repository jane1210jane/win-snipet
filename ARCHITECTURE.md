...
| 設定 | `%LOCALAPPDATA%\\SnippetTool\\settings.xml` |
| 一時設定 | `%LOCALAPPDATA%\\SnippetTool\\settings.xml.tmp` |
| バックアップ | `%LOCALAPPDATA%\\SnippetTool\\settings.xml.bak` |
| ログ | `%LOCALAPPDATA%\\SnippetTool\\logs\\app.log` |

設定ディレクトリは `SHGetKnownFolderPath(FOLDERID_LocalAppData)` から取得し、パスを固定文字列で組み立てない。
...
- 設定ファイルには現在ユーザだけが通常アクセスできる `%LOCALAPPDATA%` を使用する。追加ACL変更は行わず、継承ACLを利用する。
